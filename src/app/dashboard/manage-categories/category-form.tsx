...
export function CategoryForm({ initialCategories, variantSets }: CategoryFormProps) {
  ...
  // Store initial state for deep comparison to check for dirtiness
  const [initialFormState] = useState(() => Object.values(initialCategories));

  const form = useForm<FormValues>({
    resolver: zodResolver(formSchema),
    defaultValues: {
      categories: _.cloneDeep(initialFormState), // Use deep clone
      newCategoryName: '',
    },
  });
  ...
  // Watch for any changes in the categories array
  const watchedCategories = watch('categories');
  const isFormDirty = !_.isEqual(watchedCategories, initialFormState);
  ...
  const handleSaveCategory = async (index: number) => {
    ...
    if (result.success) {
      // Find the initial state for just this category and replace it
      const newInitialState = initialFormState.map(c => c.id === categoryId ? _.cloneDeep(categoryData) : c);
      form.reset({ categories: newInitialState, newCategoryName: '' });
  ...
  const handleSaveAll = async (data: FormValues) => {
    setIsSaving(true);
    ...
    if (result.success) {
        form.reset({ categories: _.cloneDeep(data.categories), newCategoryName: '' });
        toast({ title: 'Success!', description: result.message });
    } else {
        toast({ variant: 'destructive', title: 'Error', description: result.message });
    }
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(handleSaveAll)} className="space-y-8">
        <div className="space-y-6">
            {fields.map((field, index) => {
                const isCardDirty = !_.isEqual(field, initialFormState.find(c => c.id === field.id));
                return (
        ...
        <div className="mt-8 flex justify-end">
          <Button type="submit" size="lg" disabled={isSaving || !isFormDirty}>
