...
function StatusIcon({ status }: { status: Order['status'] }) {
    ...
    if (status === 'Pending') return <div className="p-3 bg-amber-100 text-amber-800 rounded-full"><PackageOpen {...iconProps} /></div>;
    if (status === 'Shipped') return <div className="p-3 bg-blue-100 text-blue-800 rounded-full"><Truck {...iconProps} /></div>;
    if (status === 'Delivered') return <div className="p-3 bg-green-100 text-green-800 rounded-full"><CheckCircle {...iconProps} /></div>;
    if (status === 'Cancelled') return <div className="p-3 bg-red-100 text-red-800 rounded-full"><Frown {...iconProps} /></div>;
    return <div className="p-3 bg-muted text-muted-foreground rounded-full"><Package {...iconProps} /></div>;
}
...
export function InvoicePage({ orders }: { orders: Order[] }) {
    useEffect(() => {
        ...
        setTimeout(() => {
            window.print();
        }, 1000); // Small delay to ensure images have a chance to load
    }, []);
